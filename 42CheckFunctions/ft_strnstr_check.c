...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2017/11/20 14:21:33 by ade-verd          #+#    #+#             */
/*   Updated: 2017/11/20 14:35:05 by ade-verd         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "ft_check_functions.h"
#define	FT_OFF		strnstr
#define FT			ft_strnstr

int		ft_strnstr_check(FILE *fd)
{
	int		i;
//	int		j;
	char	*src[14];
...
//Tableau de tests
	...
	while (i < 11)
	{
		off = FT_OFF(src[i], tofind[i], i);
		ft = FT(src[i], tofind[i], i);
		if (off != ft)
		{
			fprintf(stderr, "\nX_X KO\t");
			fprintf(stderr, "src : %-40s (%p)\ttofind : %-40s\tlen : %-5d\t", src[i], src[i], tofind[i], i);
			fprintf(stderr, "%s : %-40s\t%-10p\t", TO_STR(FT_OFF), off, off);
			fprintf(stderr, "ft_%s : %-40s\t%-10p\n", TO_STR(FT_OFF), ft, ft);
			fprintf(fd, "\nX_X KO\t");
			fprintf(fd, "src : %-40s (%p)\ttofind : %-40s\tlen : %-5d\t", src[i], src[i], tofind[i], i);
